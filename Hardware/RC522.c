...
u8 Wallet_Init[16]={0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x01,0xFE,0x01,0xFE};							//Value Value取反 Value 地址
u8 Recharge_Value[4] = {0x0A, 0x00, 0x00, 0x00};																																					//充值10
u8 Deduction_Value[4] = {0x05, 0x00, 0x00, 0x00};																																					//充值10
...
void MFRC_WriteReg(uint8_t addr, uint8_t data)
{
    ...
    RS522_NSS(0);                   //NSS拉低
    HardwareSPI_SwapByte(AddrByte);         //写地址字节
    HardwareSPI_SwapByte(data);             //写数据
    RS522_NSS(1);                   //NSS拉高
}
...
uint8_t MFRC_ReadReg(uint8_t addr)
{
    ...
    RS522_NSS(0);                               //NSS拉低
    HardwareSPI_SwapByte(AddrByte);             //写地址字节
    data = HardwareSPI_SwapByte(0x00);          //读数据
    RS522_NSS(1);                               //NSS拉高
    return data;
}
...
/**************************************************************************************
* 函数名称：MFRC_CmdFrame  IMPORTANT!!!
* 功能描述：MFRC522和ISO14443A卡通讯的命令帧函数
* 入口参数：-cmd：MFRC522命令字
*           -pIndata：MFRC522发送给MF1卡的数据的缓冲区首地址   1443-A 
*           -InLenByte：发送数据的字节长度
...
char PCD_Request(uint8_t RequestMode, uint8_t *pCardType)
{
    ...
    status = MFRC_CmdFrame(MFRC_TRANSCEIVE, CmdFrameBuf, 1, CmdFrameBuf, &unLen);		//MFRC_TRANSCEIVE 给RC552 CmdFrameBuf给卡片 也就是14443-A协议
...
char PCD_Anticoll(uint8_t *pSnr)
{
    ...
    MFRC_ClrBitMask(MFRC_Status2Reg, 0x08);						//清楚标志位
    MFRC_WriteReg(MFRC_BitFramingReg, 0x00);					//最后一个字节的所有位都发送，防止半个字节
    MFRC_ClrBitMask(MFRC_CollReg, 0x80);							//发生冲突清除所有标志位 

    CmdFrameBuf[0] = PICC_ANTICOLL1;									//14443 卡片防冲突
    CmdFrameBuf[1] = 0x20;

    status = MFRC_CmdFrame(MFRC_TRANSCEIVE, CmdFrameBuf, 2, CmdFrameBuf, &unLen);		//发送卡片防冲突指令
    ...
    {		
        ...
        for(i = 0; i < 4; i++)
        {
            *(pSnr + i)  = CmdFrameBuf[i];			//填充ID
						//printf("%x",CmdFrameBuf[i]);
            snr_check ^= CmdFrameBuf[i];				//填充CRC
        }
				//printf("\r\n");
    ...
    MFRC_SetBitMask(MFRC_CollReg, 0x80);						//置防冲突位
    return status;
}
...
char PCD_Select(uint8_t *pSnr)
{
    ...
    uint16_t unLen;
    uint8_t CmdFrameBuf[MFRC_MAXRLEN];
		//选卡指令0x93 0x70  93H 70H UID0-UID3 BCC 为7个字节
    CmdFrameBuf[0] = PICC_ANTICOLL1;						//防冲突
    CmdFrameBuf[1] = 0x70;
    CmdFrameBuf[6] = 0;													//补0
    ...
    MFRC_CalulateCRC(CmdFrameBuf, 7, &CmdFrameBuf[7]);	//CRC校验 

    MFRC_ClrBitMask(MFRC_Status2Reg, 0x08);

    status = MFRC_CmdFrame(MFRC_TRANSCEIVE, CmdFrameBuf, 9, CmdFrameBuf, &unLen);		//CRC 计算后为9个字节
		
		//printf("select:%x\n",CmdFrameBuf[0]);																						//打印应答成功返回值
...
char PCD_ReadBlock(uint8_t BlockAddr, uint8_t *pData)
{
    ...
    CmdFrameBuf[0] = PICC_READ;			//读指令
    CmdFrameBuf[1] = BlockAddr;			//读地址
...
* 出口参数：无
* 返 回 值：-status：错误代码(PCD_OK、PCD_NOTAGERR、PCD_ERR)
* 说    明：使用前将钱包初始化 unsigned char RFID1[16]={0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x01,0xFE,0x01,0xFE};
***************************************************************************************/
